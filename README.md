...
1. Context API는 기본으로 제공하는 상태 관리 도구로, 외부 라이브러리 설치 없이 사용 가능
2. 주로 전역 state를 관리하는데 사용된다.
3. React.createContext()로 생성한 Context 객체와 Provider 컴포넌트를 사용해 상태를 하위 컴포넌트에 전달한다.
4. 장점 : 간단하고 가볍다, 적은 설정 필요, 컴포넌트 트리의 깊이 제한 없음
5. 단점 : 복잡한 상태 관리에 한계, 성능 문제, 디버깅 도구 부족

6. Redux는 전역 상태를 관리하기 위한 독립적인 state 관리 라이브러리입니다
